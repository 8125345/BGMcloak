...
def pianoroll_to_note_sequence(chunk_func_c,
                ...
                if canWrite:
                    ...
                    if delta_time < frame_length_seconds * 1.8:
                        ...
                        note = format('{\"pitch\": %d, \"start_time\": %.5f, \"end_time\": %.5f, \"timestamp\":  '
                               ...
                               max_prob, unscale_velocity(velocity_values[i, pitch] if velocity_values else velocity))
    ...
    return sequence, sequence_client, redundant, redundant_client
...
def transcribe_chunked(argv):
    ...
    predict_files = generate_predict_set(FLAGS.input_dirs)
    logging.info('predict start! %d' % len(predict_files))
    ...
    f_out_client = open(os.path.join(FLAGS.output_client_dir, "result.txt"), "w")
    redundants = 0
    redundants_client = 0
    for wav_file in predict_files:
        try:
            _, wav_file_name = os.path.split(wav_file)
            predicted_label_midi_file = os.path.join(FLAGS.output_dir, wav_file_name + '.predicted.midi')
            predicted_label_midi_client_file = os.path.join(FLAGS.output_client_dir,
                                                            wav_file_name + '.predicted.midi')
            sequence_prediction, sequence_prediction_client, redundant, redundant_client = pianoroll_to_note_sequence(
                chunk_func,
            ...
            f_out.write(str(wav_file_name) + "\t" + str(redundant) + "\n")
            redundants += redundant
            print("redundant_client", redundant_client)
            f_out_client.write(str(wav_file_name) + "\t" + str(redundant_client) + "\n")
            redundants_client += redundant_client
        except Exception:
            print("Exception wav_file:%s" % wav_file)
            print(traceback.format_exc())
    print("redundants", redundants)
    f_out.write("all" + "\t" + str(redundants) + "\n")
    print("redundants_client", redundants)
    f_out_client.write("all" + "\t" + str(redundants_client) + "\n")
