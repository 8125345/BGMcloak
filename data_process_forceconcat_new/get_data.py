...
key_num = 88  # 琴键数量
chunk_frame_num = 32  # 模型输入帧长度
midi_frame_num = 8  # 模型输出midi序列长度
chunk_in_seg_num = int(seg_frame_num / chunk_frame_num)  # 长序列中chunk的数量

chunk_frame_num_8 = 8
chunk_in_seg_num_8 = int(seg_frame_num / 8)  # 长序列中chunk的数量
...
def split_feature_label_multiple(data):
    """
    单条数据切分为特征和标签，针对多输出模型
    :param data:
    :return:
    """
    feature = tf.stack((data[:, : n_mels], data[:, n_mels: 2 * n_mels]), axis=2)  # 32 * 229 * 2
    label_midi = data[24:, n_mels * 2:]  # 8 * 88
    label_decision = data[27: 29, n_mels * 2:]
    label_decision = tf.reduce_max(label_decision, axis=0)  # 选择两帧中最大值作为label
    label = {
        "onset_probs": label_midi,  # todo 此处需要根据模型输出确定字典名称
        "decision_output": label_decision,
    }
...
def split_feature_label(data):
    ...
    """
    feature = tf.stack((data[:, : n_mels], data[:, n_mels: 2 * n_mels]), axis=2)  # 32 * 229 * 2
    label = data[24:, n_mels * 2:]  # 8 * 88
    return feature, label


def split_feature_label_single(data):
    """
    切分特征
    :param data:
    :return:
    """
    feature = data[:, n_mels: 2 * n_mels]  # data[:, : n_mels]
    label = data[24:, n_mels * 2:]  # 8 * 88
    return feature, label


def split_feature_label_single_8(data):
    """
    8帧输入
    :param data: 8帧输入
    :return:
...
def parse_data_val_8(example_proto):
    """
    验证集中仅使用非增强数据
    :param example_proto:
    :return:
    ...
    # 单条数据扩增为多个短片段，验证集数据没有使用增强
    chunks = tf.reshape(data, (chunk_in_seg_num_8, chunk_frame_num_8, n_mels * 2 + key_num))
    # print("parse_data_val chunks", chunks.shape)
    return chunks


def parse_data_train_8(example_proto):
    """
    训练数据加载和动态增强
    :param example_proto:
    :return:
    ...
    # 随机数据裁剪，每次取用不同时间范围的数据
    data = tf.image.stateless_random_crop(value=data,
                                          size=(seg_frame_num - chunk_frame_num_8, n_mels * 2 + key_num),
                                          seed=new_seed)
    # 单条数据扩增为多个短片段
    chunks = tf.reshape(data, (chunk_in_seg_num_8 - 1, chunk_frame_num_8, n_mels * 2 + key_num))
    # print("parse_data_train chunks", chunks.shape)
    return chunks


def make_dataset(tfrecord_path_list, epoch, dataset_type, batchsize=32, shuffle=False, buffer_size=1000,
                 distributed_flag=False, distributed_strategy=None, data_split_mode="midi_map"):
    """
    数据集中包含增强信息
    ...
    assert data_split_mode in ["midi_map", "midi_map_and_decision", "single_input_midi", "single-8_input_midi"]
    ...
    if data_split_mode == "single-8_input_midi":
        parse_data_train_fun = parse_data_train_8
        parse_data_val_fun = parse_data_val_8
    else:
    ...
    if data_split_mode == "midi_map":
        split_fun = split_feature_label
    elif data_split_mode == "midi_map_and_decision":
        split_fun = split_feature_label_multiple
    elif data_split_mode == "single_input_midi":
        split_fun = split_feature_label_single
    elif data_split_mode == "single-8_input_midi":
        split_fun = split_feature_label_single_8
    else:
