...
from config_train_forceconcat_new_20220723_1 import basic_config, train_config, train_dataset
...
from data_process_forceconcat_new.get_data import make_dataset
from models_forceconcat.forceconcat_model import custom_load_model
...
if model_structure == "forceconcat" or model_structure == "doublepro":
    data_split_mode = "midi_map"
elif model_structure == "forceconcat_de":
    data_split_mode = "midi_map_and_decision"
elif model_structure == "singlepro":
    data_split_mode = "single_input_midi"
elif model_structure == "singlepro-8":
    data_split_mode = "single-8_input_midi"
else:
...
#
# # 校验路径正确性
# for item in [train_tfrecord_path_list, val_tfrecord_path_list]:
#     for path_dict in item:
#         assert os.path.exists(path_dict["path"])
# for item in [dataset_info_list]:
#     for path in item:
#         assert os.path.exists(path)

# model_path = os.path.join(model_root, model_name + "_{epoch}_{val_loss:.5f}.h5")  # 如果需要保存所有模型，可以使用这种命令格式
